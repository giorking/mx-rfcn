...
def add_bbox_regression_targets(roidb):
    ...
    num_reg_classes = 2 if config.TRAIN.AGNOSTIC else roidb[0]['gt_overlaps'].shape[1]
    ...
    if config.TRAIN.BBOX_NORMALIZATION_PRECOMPUTED:
        ...
        means = np.tile(np.array(config.TRAIN.BBOX_MEANS), (num_reg_classes, 1))
        stds = np.tile(np.array(config.TRAIN.BBOX_STDS), (num_reg_classes, 1))
    else:
        # compute mean, std values
        class_counts = np.zeros((num_reg_classes, 1)) + config.EPS
        ...
        for im_i in range(num_images):
            ...
            for cls in range(1, num_reg_classes):
                ...
                cls_indexes = np.where(targets[:, 0] > 0) if config.TRAIN.AGNOSTIC \
